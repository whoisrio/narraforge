...
| Segmented project CRUD/service | `app/api/segmented_projects.py`, `app/services/segmented_project_service.py` | `tests/test_segmented_projects_api.py`, `tests/test_segmented_projects_service.py`, `tests/integration/test_chapters_batch.py` | Project/chapter/segment persistence, orphan cleanup, voice_ref save/load and auto-migration. `chapters:batch` replace-style rebuild including per-chapter `narration_script` and per-chapter `engine` (written into `chapter.voice`); top-level `narration_script` written to file with DB storing `narration_document_path` only (same path-based storage for `source_document`). |
...
| Narration layer-sync (Phase A) | `app/services/layer_sync_service.py`, `app/services/segmented_project_service.py` (`batch_create_structure` hook), `app/api/segmented_projects.py` (split hook + `GET /sync-status`) | `tests/test_layer_sync_service.py`, `tests/test_layer_sync_integration.py`, E2E `tests/e2e/specs/layer-sync.spec.ts` | `chapter.sync_state` JSON (l1/l2/segments hashes); `mark_consistent` at batch_create, `mark_split` at split, generic save_project untouched; `sync_status` dirty detection. Frontend: `src/components/SegmentedTTS/ChapterSyncBadges.test.tsx`. |
