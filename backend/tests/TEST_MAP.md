...
| 手动导入旁白文档 + 全文视图 | `frontend/src/components/ProjectLibrary/NarrationDocView.tsx`, `frontend/src/hooks/useSegmentedProject.ts` (`SET_NARRATION_SCRIPT`), `backend/app/api/segmented_projects.py` (`PUT` `narration_script` -> `narration_document_path`; `chapters:batch` `original_text` + 旧章节音频清理), `backend/app/services/segmented_project_service.py` (`batch_create_structure`, `_delete_segment_audio_files`) | frontend `src/components/ProjectLibrary/NarrationDocView.test.tsx`, `src/components/ProjectLibrary/ProjectLibrary.test.tsx` (fulltext form A/B + modal regression), `src/components/ProjectLibrary/ChapterSplitModal.test.tsx` (正文剩标题 + 确认弹窗), `src/hooks/__tests__/useSegmentedProject.test.ts` (`SET_NARRATION_SCRIPT`), backend `tests/integration/test_chapters_batch.py` (`test_batch_persists_chapter_original_text`, `test_batch_deletes_existing_chapter_audio`), E2E `tests/e2e/specs/narration-manual-import-split.spec.ts` | 全文视图自适应：`narration_script` 空 = 章节合并只读预览 + 转换入口（粘贴旁白文档 / 从现有章节生成）；非空 = 编辑/预览切换 + 拆分按钮。拆分剩标题行得到正文，`original_text`+`narration_script` 都落正文（章节卡片有内容、studio 可拆）；已有章节时点应用弹确认（alertdialog）再替换，替换时清理旧章节音频文件。后端零接口改动（复用 `ProjectIn.narration_script`）。 |
