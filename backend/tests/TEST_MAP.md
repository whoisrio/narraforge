...
| Chapter & segment reorder | `frontend/src/hooks/useSegmentedProject.ts` (`MOVE_CHAPTER` action, `REORDER` renumbers `position`), `frontend/src/components/ProjectShell/ProjectShell.tsx` (chapter ↑/↓), `frontend/src/components/SegmentedTTS/SegmentRow.tsx` + `SegmentList.tsx` (segment ↑/↓ -> `onReorder`), `frontend/src/pages/TTSSynthesis.tsx` (wiring), `backend/app/services/segmented_project_service.py` (`save_project` reconciles `position` from payload, no new endpoint) | frontend `src/hooks/__tests__/useSegmentedProject.test.ts` (`MOVE_CHAPTER` order+renumber, `REORDER` renumber, boundary no-op), `src/components/ProjectShell/ProjectShell.test.tsx` (chapter move buttons + boundaries + absent-when-no-callback), `src/components/SegmentedTTS/SegmentList.reorder.test.tsx` (segment move buttons -> `onReorder(from,to)` + boundaries + single-segment hidden), E2E `tests/e2e/specs/reorder.spec.ts` | 章节侧边栏与段落行各加 ↑/↓ 按钮；前端 reducer 重排数组后按索引重写 `position`（后端 `save_project` 信任 `position`，不重写会被旧值静默回滚，故必须重编号）；乐观更新 + 防抖自动保存 PUT 全量项目；E2E 双读校验 UI→API→DB `position` 三层一致并在用例末还原顺序。后端零接口改动。 |
