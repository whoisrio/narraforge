...
| Qwen/CosyVoice TTS API | `app/api/tts.py`, `app/services/qwen_tts_service.py`, `app/schemas/tts.py` (`TTSResultOut`/`TTSHistoryOut`) | `tests/integration/test_tts_api.py`, `tests/test_api_tts.py`, `tests/unit/test_qwen_tts_service.py` | Current contract: Qwen/CosyVoice synthesis returns an audio file path. B-P1-8b: `response_model=TTSResultOut` on `/synthesize`; `/history` -> `TTSHistoryOut`. |
