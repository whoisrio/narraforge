...
- `knowledge_video` is the second registered assistant (`agent/app/graph_knowledge_video.py`): source document → faithful narration transcription → basic quality review (human confirm) → chapter split (rule-based punctuation split + LLM merge/split/emotion review, source text never rewritten) → engine-aware synthesis (mimo→冰糖预置 / voxcpm→`VOXCPM_DEFAULT_ROLE_ID` clone / edge_tts default) → Remotion project scaffolding. The scaffold root is a global backend-DB setting (editable from `/settings`), resolved as `target_dir` override > project `remotion_project_path` > `{animation_root_folder}/{safe_project_name}`. Per-chapter audio + SRT are exported. TTS failures halt before scaffold. The frontend drawer selects the assistant by workflow kind (`WORKFLOW_KINDS` mapping).
...
Cross-stack browser E2E tests live under `tests/e2e/`. All 45 tests pass with `npm run e2e`.
