...
All backend tests in `backend/tests/` directory. Run:
```bash
cd backend && uv run pytest
```
All frontend test files (`.test.ts` / `.test.tsx` / `.spec.ts` / `.spec.tsx`) **must be placed in the same directory as the source code they test
