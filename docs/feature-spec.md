...
Rules: at synthesis time `prepare_text_for_engine` strips unsupported tags and injects the leading tag (emotion first, style after, comma-joined); a chapter-level **mute_tags** switch ("禁用风格 tag", recommended for clone voices) forces full stripping; a chapter-level **underscore_to_space** switch ("合成时把下划线转为空格") replaces `_` with a space in the text sent to the engine only — display, subtitle exports, and stored history keep the original text; a project-level global switch in Project Settings (`configs.underscore_to_space`) does the same project-wide, and either switch being on enables the conversion; a **skip_parenthesized** switch ("忽略括号内容", chapter-level and project-level `configs.skip_parenthesized`, either-on) removes matched half-width `()` / full-width `（）` pairs including their contents from the text sent to the engine only (non-nested pairs; unmatched brackets kept), applied before tag adaptation so an engine-injected leading `(情绪)` tag is never eaten; both ignore toggles are grouped under an expandable「合成忽略选项」section in the chapter sidebar and in Project Settings; SRT/subtitle exports always strip all tags. The narration workflow asks for the TTS engine (countdown interrupt with a default before segment splitting) and the LLM injects inline tags only when VoxCPM is selected. Tags can also be inserted manually in the segment editor via the tag inserter. Emotion chips render with per-emotion colors in both compact and expanded studio views.
