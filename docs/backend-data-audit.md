...
| A11 | **List envelope inconsistency**: bare arrays (`/clone/list`, `/config/models`, `/segmented-projects`, `/roles`) vs wrapped objects (`{"voices": ...}`, `{"results": ...}`, `{"languages": ...}`); no `{items, total}` anywhere. | `clone.py:625`, `config.py:48`, `tts.py:333,441,464`, `speech_to_text.py:204` |
| A12 | **Agent → backend contract is untyped**: `backend_client.py` returns raw `r.json()` dicts (`:42-47,110-129,131-141`) and hand-picks fields instead of `model_validate` (`:83-89`) — a backend rename silently becomes `None`. | `agent/app/backend_client.py` |
