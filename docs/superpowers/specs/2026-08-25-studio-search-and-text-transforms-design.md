...
- 发音映射与大写转小写都走「**先搜出命中的 segment → 再选择应用范围**」的交互流程，并各自提供「无脑全局生效」选项（映射的 `apply_all` 开关 / 项目级小写默认开关）。
...
- `apply_all=true` 即「无脑全局映射」：该映射对生效范围内的所有段自动生效，无需逐段勾选。
  - 全局条目的 `apply_all` 对**所有项目**生效。
  - 项目条目的 `apply_all` 仅对**本项目**生效。
...
- 生效字典 = 全局 ∪ 项目，以 `source` 为键合并，**同 `source` 项目条目整体覆盖全局条目**（含其 `apply_all` 与 `id`）。
...
3. 或者直接打开该条目的 `apply_all` 开关，跳过逐段勾选，全项目（或全局）生效。
...
effective = [e for e in merged if e.apply_all or e.id in seg.applied_map_ids]
...
- `apply_all` 条目对不含 `source` 的段天然是无副作用的 no-op。

## 功能 3：大写词转小写

### 数据模型

- 项目默认：`project.configs.lowercase_latin: bool`（「无脑全局」开关）。
...
| `src/components/SegmentedTTS/PronunciationMapPanel.tsx`（新增） | 项目字典 CRUD、全局字典只读展示（带「全局」徽标）、命中段勾选列表、apply_all 开关、替换效果预览 |
...
- `text_transform_service`：合并覆盖（同 source 项目赢）、`apply_all` 生效与范围、长度降序替换、单次替换不递归、`[A-Z]{2,}` 边界（`I` 不变 / `API` 变 / 中英混排 / `target` 含大写词被小写化）、悬空 id 忽略。
...
- `PronunciationMapPanel`：CRUD、命中段勾选写回 `applied_map_ids`、apply_all、删除引用清理、替换效果预览（镜像与后端共享测试夹具保证一致）。
...
- `apply_all` 无脑流程：开全局 apply_all 映射 → 任意段合成 → 断言替换生效。
