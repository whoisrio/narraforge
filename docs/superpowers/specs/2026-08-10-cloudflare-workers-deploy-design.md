...
Cloudflare 版只保留在线服务能力：TTS 全部走在线 API（edge-tts / 小米 mimo / dashscope cosyvoice），不提供本地模型 TTS（voxcpm）和语音转字幕（whisper/funasr），不部署 LangGraph agent 工作流。
...
保留的路由（全部纯在线或纯数据）：
`tts.py`（edge-tts + qwen/dashscope）、`mimo_tts.py`、`clone.py`（dashscope/mimo 在线克隆，voxcpm 子端点禁用）、`subtitle_llm.py`、`text_split.py`、`text_analysis.py`、`config.py`、`model_config.py`、`roles.py`、`sources.py`、`segmented_projects.py` 的元数据 CRUD 部分。
...
### 3.4 mimo / dashscope 走纯 REST

`mimo_tts_service.py` 把 `urllib.request` 换成 httpx（spike 已验证 httpx 在 Pyodide 可用，且本地 CPython 同样可用，一份代码两处跑）。
`qwen_tts_service.py` 不移植 dashscope SDK 到 workers，workers 模式直接调 dashscope REST API（`HttpSpeechSynthesizer` 对应的 HTTP 端点 + `VoiceEnrollmentService` 的 REST 端点），本地模式可保留 SDK。
声音克隆的公网音频 URL 需求沿用七牛上传：workers 模式用 httpx 直调七牛 REST，本地模式保留 qiniu SDK。
...
本地版无鉴权，公网部署必须加，否则任何人能消耗 dashscope/mimo 额度。
workers 模式启用一个共享密钥校验：`Authorization: Bearer <token>`，token 存 Workers secret，前端首次访问输入后存 localStorage，axios 拦截器统一带上。
...
- 新增运行时能力探测：`GET /api/config/capabilities` 返回 `{ deploy_target, engines: [...], features: { speech_to_text, agent_workflow, backend_storage } }`。
  前端据此隐藏/禁用：SpeechToText 页入口、VoiceClone 的 voxcpm 引擎选项、TTS 面板的 voxcpm/cosyvoice-local 分支、工作流 drawer 入口、backend 存储模式切换。
...
### 5.2 后端（Workers）
- `backend/wrangler.toml`：python_workers 兼容标志，`compatibility_date >= 2025-11-02`（spike 验证 2025-08-01 有 `on_fetch` 报错），入口 `workers_entry.py`，绑定 R2 bucket，secrets 存 Supabase service key、克隆用七牛/dashscope key、访问 token。
...
2. Workers 校验 token → 按引擎分发：edge-tts 走内置 WS 客户端 / mimo、dashscope 走 httpx REST。
3. 音频以 base64 返回前端，前端存 IndexedDB（frontend 存储模式，音频不经 Workers 持久化）。
4. 项目/章节/分段元数据经 `SupabaseRepository` 写 PostgREST；音色样本/试听音频写 R2。
...
| dashscope REST 绕过 SDK 后参数漂移 | 对照 SDK 源码写请求，集成测试标记 external 真实调一次 |
...
2. mimo/edge-tts/qwen 网络层 httpx 化与策略化。
3. 仓储接口 + SupabaseRepository + R2 资产存储。
4. Workers 入口 + wrangler.toml + 鉴权 + CORS。
5. 前端 capabilities + baseURL 环境变量 + axios 拦截器。
