...
| `TRY_ANON_DAILY_LIMIT` | No | Try page (`/try`) anonymous rate limit (workers mode, anonymous requests only): edge_tts syntheses per IP per day (UTC), enforced via the `rate_limit_counters` Supabase table; exceeding returns `429 rate_limit_exceeded`. Authenticated users and local mode are never limited. Requires the `rate_limit_counters` table + `hit_rate_limit` RPC from `backend/supabase/schema.sql` | `50` |
| `MAX_SEGMENT_CHARS` | No | Segment text length limit (both deploy modes — a content-quality constraint, not a billing quota): over-long split results are truncated at the nearest punctuation within the limit (hard cut when no punctuation is in range); full-save (`POST/PUT /api/segmented-projects`, `chapters:batch`) and segment-synthesis text overrides carrying over-long text are rejected with `422 segment_too_long`. `0` or negative disables the limit | `80` |
| `MAX_CHAPTERS_PER_PROJECT` | No | Per-project chapter quota (workers mode, regular logged-in users only): growth is rejected with `409 chapter_limit_reached` when the incoming chapter count exceeds both this limit and the project's current chapter count (existing over-cap projects can still save but not grow). `0` disables the quota. Legacy admins and `ADMIN_EMAILS` users are exempt. Local mode never enforces it | `3` |
