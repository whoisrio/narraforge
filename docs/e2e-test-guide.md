...
```bash
npm run e2e          # Full suite (59 tests, --workers=1, HTML report)
...
| Path | Purpose |
|---|---|
| `tests/e2e/specs/` | Automated browser E2E specs (59 tests, all Chinese locale) |
...
```text
tests/e2e/
├── specs/                  ← Automated Playwright browser specs (59 tests)
...
- All 59 tests set `setLocaleToZhCN(page)` and use Chinese test names.
...
All 59 current tests pass. The following scenarios are not yet covered.
...
| G5 | **English locale UI** | All 59 tests use Chinese locale; no English locale coverage | New locale-parameterized spec or standalone `i18n-en.spec.ts` | Medium |
