...
```bash
npm run e2e          # Full suite (31 tests, --workers=1, HTML report)
...
| Path | Purpose |
|---|---|
| `tests/e2e/specs/` | Automated browser E2E specs (31 tests, all Chinese locale) |
...
```text
tests/e2e/
├── specs/                  ← Automated Playwright browser specs (31 tests)
...
- All 31 tests set `setLocaleToZhCN(page)` and use Chinese test names.
...
| G5 | **English locale UI** | All 31 tests use Chinese locale; no English locale coverage | New locale-parameterized spec or standalone `i18n-en.spec.ts` | Medium |
