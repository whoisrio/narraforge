...
```bash
npm run e2e          # Full suite (56 tests, --workers=1, HTML report)
...
- All 56 tests set `setLocaleToZhCN(page)` and use Chinese test names.
...
All 56 current tests pass. The following scenarios are not yet covered.
...
| G5 | **English locale UI** | All 56 tests use Chinese locale; no English locale coverage | New locale-parameterized spec or standalone `i18n-en.spec.ts` | Medium |
