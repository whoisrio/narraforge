...
```bash
npm run e2e          # Full suite (37 tests, --workers=1, HTML report)
...
| Path | Purpose |
|---|---|
| `tests/e2e/specs/` | Automated browser E2E specs (37 tests, all Chinese locale) |
...
```text
tests/e2e/
├── specs/                  ← Automated Playwright browser specs (37 tests)
...
- All 37 tests set `setLocaleToZhCN(page)` and use Chinese test names.
...
| G5 | **English locale UI** | All 37 tests use Chinese locale; no English locale coverage | New locale-parameterized spec or standalone `i18n-en.spec.ts` | Medium |
