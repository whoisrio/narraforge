...
| Path | Purpose |
|---|---|
| `tests/e2e/specs/` | Automated browser E2E specs (47 tests, all Chinese locale) |
...
```text
tests/e2e/
├── specs/                  ← Automated Playwright browser specs (47 tests)
...
- All 47 tests set `setLocaleToZhCN(page)` and use Chinese test names.
...
| G5 | **English locale UI** | All 47 tests use Chinese locale; no English locale coverage | New locale-parameterized spec or standalone `i18n-en.spec.ts` | Medium |
