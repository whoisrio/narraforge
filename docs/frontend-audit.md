...
🔄 2026-08-21 — re-unified around saved-locale-first: all three entry points now resolve via `detectInitialLocale()` (`localStorage['narraforge-locale']` → `DEFAULT_LOCALE`), and `DEFAULT_LOCALE` flipped to `en-US` for the online acquisition deployment.
Unit-test setup pins zh-CN (`src/test/setup.ts`, including a `Storage.prototype.clear` patch); E2E pins zh-CN via `tests/e2e/helpers/locale.ts`. |
...
- ✅ 2026-08-19 — Library 文档优先重构顺手清理三项：装饰性 filter chips 行（「进行中/草稿/完成」无交互无实义）已随 chapters 视图重构删除；tab「分镜」硬编码中文随分镜入口移除退场（新切换器三键全部走 i18n key：`projectLibrary.viewDoc/viewChapters/viewSource`）；`ProjectLibrary.tsx` 头部注释掉的标题代码块已删除。
