...
| # | Finding | Evidence |
|---|---------|----------|
| I1 | **28 `t()` keys referenced in source do not exist in the language packs.** At runtime the raw key string (e.g. `tts.playFailed`) is rendered, including in confirmation dialogs for destructive operations. Most are wrong paths to synonyms that exist elsewhere (e.g. `projectLibrary.newChapter` exists but components reference `tts.newChapter`). | `pages/TTSSynthesis.tsx` (17 keys: `:411`, `:441`, `:572`, `:652-653`, `:691`, `:834`, `:1172`, `:1227`, `:1268`, `:1273`, `:1621`, `:1625`), `RoleSyncPrompt.tsx:17,22`, `SegmentRow.tsx:530,539`, `ProjectOverview.tsx:160`, `ProjectHub.tsx:91`, `ProjectLibrary.tsx:475`, `VoiceClone.tsx:469`, `ProjectVoices.tsx:782,860` | ✅ 2026-08-05 — `missing-keys.test.ts` 3/3 pass; all 28 keys resolved by prior PRs (i18n key additions across F-P0-3, I2, I3, I4 work). |
<<<<<<< HEAD
| I2 | **The Workflow module is not wired to i18n.** 7 of 8 components under `components/Workflow/` hardcode Chinese (only `ReviewEditor.tsx` uses `useTranslation`). `GlobalControlBar.tsx` and `SynthesisHistory.tsx` are also fully hardcoded. ~90+ lines of UI copy; English users see a largely Chinese UI. | `WorkflowDrawer.tsx:195,218-231,249,252`, `SynthesisHistory.tsx:31-34,95-98,118-182`, `GlobalControlBar.tsx:72-218` | ✅ 2026-08-05 — All Workflow components wired (6/6 remaining). GlobalControlBar wired. SynthesisHistory already deleted (F-P0-4 dead code). Added 20+ i18n keys. |
<<<<<<< HEAD
| I3 | **9 files import the module-level `t`, which is pinned to zh-CN and never reacts to locale switches.** Module-level constants evaluated with it (e.g. tab labels) never update on language change. | `MiMoTTSPanel.tsx:12,42-43`, `ProjectVoices.tsx:12`, `SSMLToolbar.tsx:13`, `NarrationBlock.tsx:3`, `TTSSynthesis.tsx:2` (`staticT`), `useSegmentedProject.ts:2`, `voiceRoleDefaults.ts:2`, `voiceDesignPreview.ts:3`, `voiceRolePreview.ts:3` | ✅ 2026-08-05 — 7/9 fixed with `translate` param threading: MiMoTTSPanel (MODE_TABS→component), ProjectVoices (createRoleDraft→param, 3 call sites pass t), NarrationBlock (unused import removed), useSegmentedProject (migrateV1→param, reducer reads localStorage locale), TTSSynthesis (7 call sites pass t), VoiceClone (1 call site passes t), voiceDesignPreview/voiceRolePreview (→translate param with callers wired). Dead `roleVoiceLabelFromParams` deleted. SSMLToolbar+TTSSynthesis staticT deferred (module-level constants). |
=======
| I3 | **9 files import the module-level `t`, which is pinned to zh-CN and never reacts to locale switches.** Module-level constants evaluated with it (e.g. tab labels) never update on language change. | `MiMoTTSPanel.tsx:12,42-43`, `ProjectVoices.tsx:12`, `SSMLToolbar.tsx:13`, `NarrationBlock.tsx:3`, `TTSSynthesis.tsx:2` (`staticT`), `useSegmentedProject.ts:2`, `voiceRoleDefaults.ts:2`, `voiceDesignPreview.ts:3`, `voiceRolePreview.ts:3` | ✅ 2026-08-05 — All 9 files fixed: MiMoTTSPanel (MODE_TABS→component), ProjectVoices (createRoleDraft→param, 3 call sites pass t), NarrationBlock (unused import removed), useSegmentedProject (migrateV1→param, reducer reads localStorage locale), TTSSynthesis (7 call sites pass t, staticT→createTranslator), VoiceClone (1 call site passes t), voiceDesignPreview/voiceRolePreview (→translate param with callers wired), SSMLToolbar (→createTranslator for module constants). Dead `roleVoiceLabelFromParams` deleted. |
>>>>>>> bc8624b (fix: I3 remaining (SSMLToolbar+staticT) + A12 (agent model_validate))
=======
| I2 | **The Workflow module is not wired to i18n.** 7 of 8 components under `components/Workflow/` hardcode Chinese (only `ReviewEditor.tsx` uses `useTranslation`). `GlobalControlBar.tsx` and `SynthesisHistory.tsx` are also fully hardcoded. ~90+ lines of UI copy; English users see a largely Chinese UI. | `WorkflowDrawer.tsx:195,218-231,249,252`, `SynthesisHistory.tsx:31-34,95-98,118-182`, `GlobalControlBar.tsx:72-218` | ✅ 2026-08-05 — 7/8 Workflow components wired (DrawerIndicator, StageLLMPane, StageCard, WorkflowDrawer, EngineSelectPanel, ConfirmPanel, ReviewPanel). GlobalControlBar wired. SynthesisHistory already deleted (F-P0-4 dead code). Added 20+ i18n keys. |
| I3 | **9 files import the module-level `t`, which is pinned to zh-CN and never reacts to locale switches.** Module-level constants evaluated with it (e.g. tab labels) never update on language change. | `MiMoTTSPanel.tsx:12,42-43`, `ProjectVoices.tsx:12`, `SSMLToolbar.tsx:13`, `NarrationBlock.tsx:3`, `TTSSynthesis.tsx:2` (`staticT`), `useSegmentedProject.ts:2`, `voiceRoleDefaults.ts:2`, `voiceDesignPreview.ts:3`, `voiceRolePreview.ts:3` | ✅ 2026-08-05 — All 9 files fixed: MiMoTTSPanel (MODE_TABS→component), ProjectVoices (createRoleDraft→param, 3 call sites pass t), NarrationBlock (unused import removed), useSegmentedProject (migrateV1→param, reducer reads localStorage locale), TTSSynthesis (7 call sites pass t, staticT→createTranslator), VoiceClone (1 call site passes t), voiceDesignPreview/voiceRolePreview (→translate param with callers wired), SSMLToolbar (→createTranslator for module constants). Dead `roleVoiceLabelFromParams` deleted. |
>>>>>>> master
>>>>>>> master
| I4 | **Default locale is inconsistent in three places.** Provider defaults to `en-US`, module-level `t` is fixed at `zh-CN`, and the no-Provider hook fallback is `zh-CN`. | `i18n/index.tsx:69,85,113` | ✅ 2026-08-05 — unified to `zh-CN` (TranslationProvider default changed from `en-US` to `zh-CN`) |
...
- F-P1-5 converge the 14 hand-rolled overlays onto `ui/Modal` + route z-index through tokens - large mechanical change. **Skipped 2026-08-03 (pending visual verification):** full convergence changes every dialog's visuals (`ui/Modal` is generic/full-width vs custom widths/headers) and needs a `maxWidth` prop on `Modal`; z-index token migration has stacking-context edge cases (lowering overlays below the hand-rolled `z-index:1000` ones broke click interception - see layer-sync E2E). E2E can't catch pixel regressions, so this needs ego-browser visual verification before doing. Revisit with browser-driven review.
