...
export function SourceLibrary() {
  ...
  const handleAddSources = (newSources: SourceDocument[]) => {
    ...
    showToast(t('sourceLibrary.sourcesAdded', { count: newSources.length }), 'success');
  };
  ...
  const confirmDeleteAction = () => {
    ...
    showToast(t('sourceLibrary.sourceDeleted', { title: confirmDelete.title }), 'success');
  ...
  const handleSwitchVersion = (version: string) => {
    ...
    showToast(t('sourceLibrary.switchedToVersion', { version }), 'success');
  };
  ...
  const handleCreateProject = () => {
    ...
    showToast(t('sourceLibrary.projectCreated', { name }), 'success');
  };
    ...
    <div className={styles.container}>
      <div className={styles.layout}>
        ...
        <aside className={styles.sidebar}>
          <div className={styles.sidebarHeader}>
            ...
            <h2 className={styles.sidebarTitle}>{t('sourceLibrary.projects')}</h2>
          </div>
          <button className={styles.newProjectBtn} onClick={handleCreateProject}>
            <span>+</span> {t('sourceLibrary.newProjectBtn')}
          </button>
          <div className={styles.projectList}>
            {MOCK_PROJECTS.map(project => {
              ...
              return (
                ...
                >
                  <span className={styles.projectIcon}>{project.id === '__scratchpad__' ? t('sourceLibrary.scratchpadIcon') : project.name.charAt(0)}</span>
        ...
        <main className={styles.main}>
          <div className={styles.mainHeader}>
            <div className={styles.titleCluster}>
              ...
              <h1 className={styles.mainTitle}>{activeProject.name} · {t('sourceLibrary.sourceLibrarySuffix')}</h1>
            </div>
            {!isScratchpad && (
              <div className={styles.headerActions}>
                <button
                  ...
                  title={activeSources.length === 0 ? t('sourceLibrary.pleaseAddSourcesFirst') : t('sourceLibrary.generateNarrationTooltip')}
                >
                  🧠 {t('sourceLibrary.generateNarration')}
                </button>
              </div>
            )}
          </div>
          ...
          {isScratchpad ? (
            <div className={styles.scratchpadNotice}>
              ...
              <div>
                <div className={styles.noticeTitle}>{t('sourceLibrary.scratchpadNoSourceManagement')}</div>
                <div className={styles.noticeDesc}>{t('sourceLibrary.scratchpadNoticeDesc')}</div>
              </div>
            </div>
          ) : (
            <div className={styles.workspace}>
              ...
              <section className={styles.section}>
                <div className={styles.sectionHeader}>
                  <h2 className={styles.sectionTitle}>📚 {t('sourceLibrary.sourcesSection')} · {activeSources.length}</h2>
                  <span className={styles.sectionHint}>
                    {t('sourceLibrary.sourceHint')}
                  </span>
                </div>
                {activeSources.length > 0 ? (
                      ...
                      <div key={src.id} className={styles.sourceCard}>
                        <div className={styles.sourceCardHeader}>
                          ...
                          <div className={styles.sourceCardBody}>
                            ...
                            <div className={styles.sourceCardMeta}>
                              {src.source_type === 'paste' && src.pasted_text && `${src.pasted_text.length} ${t('sourceLibrary.characters')}`}
                              {src.source_type === 'audio' && src.duration_sec && formatDuration(src.duration_sec)}
                              {src.source_type === 'path' && t('sourceLibrary.localPathRef')}
                              {t('sourceLibrary.addedOn')}
                          ...
                          <button
                            ...
                            title={t('sourceLibrary.deleteSource')}
                          >
                            ×
                          </button>
                        </div>
                        {src.source_type === 'paste' && src.pasted_text && (
                          <>
                            ...
                            <div className={styles.sourceActions}>
                              ...
                              >
                                {analyzingSourceId === src.id ? t('sourceLibrary.analyzing') : `🔍 ${t('sourceLibrary.smartSplitChapters')}`}
                              </button>
                            </div>
                          </>
                        )}
                        {src.source_type === 'audio' && (
                          <div className={styles.sourceAudioRow}>
                            <button className={styles.audioPlayBtn} disabled>▶ {t('sourceLibrary.preview')}</button>
                            <span className={styles.audioStatus}>{t('sourceLibrary.uploaded')}</span>
                          </div>
                        )}
                      </div>
                    ))}
                  ...
                  <div className={styles.emptyState}>
                    ...
                    <div className={styles.emptyTitle}>{t('sourceLibrary.noSourcesYet')}</div>
                    <div className={styles.emptyDesc}>{t('sourceLibrary.noSourcesDesc')}</div>
                  </div>
                )}
                <SourceUploadZone
              ...
              <section className={styles.section}>
                ...
                {activeNarrations.length > 0 ? (
                    ...
                    {currentNarration && (
                      <div className={`${styles.narrationCard} ${styles.narrationCardActive}`}>
                        <div className={styles.narrationCardHeader}>
                          <div className={styles.narrationVersionBlock}>
                            ...
                            <span className={styles.narrationKind}>
                              {currentNarration.version_kind === 'full' ? t('sourceLibrary.fullScript') : t('sourceLibrary.chapterFork')}
                            </span>
                            <span className={styles.narrationActiveTag}>{t('sourceLibrary.active')}</span>
                          </div>
                          <div className={styles.narrationMeta}>
                            {t('sourceLibrary.chaptersWords', { chapters: (currentNarration.chapter_slices || []).length, words: currentNarration.word_count.toLocaleString() })}
                          </div>
                          <div className={styles.narrationActions}>
                            ...
                            >
                              👁 {t('sourceLibrary.fullText')}
                            </button>
                            <button
                              className={styles.actionBtn}
                              onClick={() => {
                                navigator.clipboard.writeText(currentNarration.body_markdown);
                                showToast(t('sourceLibrary.markdownCopied'), 'success');
                              }}
                            >
                              📋 {t('common.copy')}
                            </button>
                        ...
                        <div className={styles.narrationPreview}>
                          {currentNarration.body_markdown.slice(0, 200).replace(/^#+\s.*$/gm, '').trim() || t('sourceLibrary.emptyDocument')}
                          {currentNarration.body_markdown.length > 200 ? '...' : ''}
                        </div>
                        <div className={styles.narrationFooter}>
                          <span>{t('sourceLibrary.basedOnNSources', { count: currentNarration.source_ids.length })}</span>
                          {currentNarration.prompt_hint && (
                            <span className={styles.promptHint}>{t('sourceLibrary.promptHint')}{currentNarration.prompt_hint}"</span>
                          )}
                    ...
                    {activeNarrations.length > 1 && (
                      <details className={styles.historyBlock}>
                        <summary className={styles.historySummary}>
                          📚 {t('sourceLibrary.historyVersions')} ({activeNarrations.length - 1} 个)
                          <span className={styles.historyHint}>{t('sourceLibrary.clickToExpand')}</span>
                        </summary>
                              ...
                              <div key={narr.id} className={styles.historyItem}>
                                <div className={styles.historyItemLeft}>
                                  ...
                                  <span className={styles.historyKind}>
                                    {narr.version_kind === 'full' ? t('sourceLibrary.fullScript') : t('sourceLibrary.chapterFork')}
                                  </span>
                                  <span className={styles.historyMeta}>
                                    {t('sourceLibrary.chaptersWords', { chapters: (narr.chapter_slices || []).length, words: narr.word_count.toLocaleString() })}
                                  </span>
                                ...
                                <div className={styles.historyItemActions}>
                                  ...
                                  >
                                    {t('sourceLibrary.switchToThisVersion')}
                                  </button>
                                </div>
                              </div>
                            ))}
                  ...
                  <div className={styles.emptyState}>
                    <div className={styles.emptyIcon}>📜</div>
                    <div className={styles.emptyTitle}>{t('sourceLibrary.noNarrationDocs')}</div>
                    <div className={styles.emptyDesc}>
                      {activeSources.length === 0
                        ? t('sourceLibrary.addSourcesFirst')
                        : t('sourceLibrary.clickGenerateToStart')}
                    </div>
                  </div>
                )}
              </section>
      ...
      {confirmDelete && (
        <ConfirmDialog
          open
          title={t('sourceLibrary.deleteSource')}
          message={t('sourceLibrary.deleteSourceConfirm', { title: confirmDelete.title })}
          variant="warning"
      ...
      {(analysisResult !== null || analyzingSourceId !== null) && (
        <ScriptAnalysisModal
          ...
          onConfirm={() => {
            showToast(t('sourceLibrary.chaptersRolesIdentified'), 'success');
            handleCloseAnalysis();
          }}
        />
      )}
